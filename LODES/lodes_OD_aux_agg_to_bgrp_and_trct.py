...
#Script to aggregate the OD aux data from Lodes7 2015 files data. 
...
# We use the allstates file to join with, as aux is over state movement. 
filename_xwalk='/Users/maarten/Desktop/nyu-data/LODES/xwalk/all_states_xwalk_small.gzip' 

print 'Started reading in xwalk data for all states '	
#Make sure to read our fiels as string.
dtype_dict_xwalk={'tabblk2010': object,'trct': object,'bgrp': object}	
df_xwalk = pd.read_csv(filename_xwalk,sep=',', lineterminator='\n',dtype=dtype_dict_xwalk,compression='gzip')
df_xwalk_small=df_xwalk[['tabblk2010','trct','bgrp']]
print 'Ended reading in xwalk data for all states ' 
...
for statename in statename_list:
	...
	filename='/Users/maarten/Desktop/nyu-data/LODES/OD/aux_JT00_2015_source/%s_od_aux_JT00_2015.csv.gz' %statename
	if statename=='wy':
		filename='/Users/maarten/Desktop/nyu-data/LODES/OD/aux_JT00_2015_source/%s_od_aux_JT00_2013.csv.gz' %statename
		# WY only has data for 2013 (all the rest is 2015)	
	...
	# make copy so we surely don't mess with the original data
	df_xwalk_small_copy = df_xwalk_small.copy()

	df_od_xwalk_tussen=pd.merge(df_od,df_xwalk_small_copy, how='left', left_on='w_geocode', right_on ='tabblk2010')
	...
	df_od_xwalk=pd.merge(df_od_xwalk_tussen,df_xwalk_small_copy, how='left', left_on='h_geocode', right_on ='tabblk2010')
	...
	for agg_level in agg_level_list:
		...
		outputname='/Users/maarten/Desktop/nyu-data/LODES/OD/aux_JT00_2015_agg_%s/%s_od_aux_JT00_2015_agg_%s.csv.gz' %(agg_level,statename, agg_level)

		if statename=='wy':
			outputname='/Users/maarten/Desktop/nyu-data/LODES/OD/aux_JT00_2015_agg_%s/%s_od_aux_JT00_2013_agg_%s.csv.gz' %(agg_level,statename, agg_level)
