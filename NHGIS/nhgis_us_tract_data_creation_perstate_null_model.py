...
# For a null model that bases on each state separately
...
r_df_by_state={}
for state in r_dict_by_state.keys():
    print state
    r_df_state=convert_r_dict_to_df(r_dict_by_state[state])
    r_df_by_state[state]=r_df_state
print 'Ended conversion of r_dicts to panda DataFrame'

#for state in r_df_by_state.keys():
#    print state, r_df_by_state[state].size
#r_df_by_state['01'].head(10)

#####################
#4.2 Add information on statistically significant over- or under representation
#####################
...
###############
#4.2.1 Calculate z-score for r_values based on null model of each state
###############

print 'Started calculation of z-scores for representation on null model for each state'
for state in r_df_by_state.keys():
    r_df_by_state[state]['z_score']=(r_df_by_state[state]['r_value'] - avg_exp_value_for_null_model) / pow(r_df_by_state[state]['r_variance_null_model'],0.5)
print 'Ended calculation of z-scores for representation on null model for each state'


###############
#4.2.2 Evaluate values at 99% interval based on null model for each state
###############E

print 'Started evaluation of representation values for the 99 procent interval based on null model for each state'
...
for state in r_df_by_state.keys():
    r_df_by_state[state]['signif_cat_99'] = r_df_by_state[state].apply(r_value_to_signif_cat,args=(confidence_interval_factor,avg_exp_value_for_null_model),axis=1)

print 'Ended evaluation of representation values for the 99 procent interval based on null model for each state'


#####################
#4.3 Unfold r_df_by_state to one long list r_df_allstates and save this to pickle
#####################

#Flush out one large df, collecting the information of all states. 
print 'Started unfolding r_df_by_states to flush to pickle as one long list'
states_processed=0

for state in r_df_by_state.keys():
	#Append to masterdf 'r_df_all_states'
	if states_processed == 0: 
		print 'This is the first state: ' + str(state)
		states_processed=states_processed+1
		r_df_by_state[state]['STATEA']=state #recreate state field -- we dropped this before to create the marble dicts.
		r_df_all_states=r_df_by_state[state]
		print 'resulting shape: ' +str(r_df_all_states.shape)

	else:
		print 'We are adding state: ' + str(state)
		r_df_by_state[state]['STATEA']=state #recreate state field -- we dropped this before to create the marble dicts.
		r_df_all_states=r_df_all_states.append(r_df_by_state[state], ignore_index=True)
		states_processed=states_processed+1 
		print 'appending shape: ' +str(r_df_by_state[state].shape)
		print 'resulting shape: ' +str(r_df_all_states.shape)
print 'Ended unfloding r_df_by_states to flush to pickle as one long list'


print 'Starting saving of r_df_all_states to pickle'
foldername ='/Users/maarten/Desktop/nyu-data/NHGIS/us/'
...
r_df_all_states.to_pickle(picklename)
print 'Ended saving of r_df_all_states to pickle'


#####################
#4.4 Calculate one measure of segregation, weighting representation values (z-scores) with population numbers
...
#r_df_all_states_pivot=r_df_all_states.pivot(index='area_id', columns='class_id', values='r_value').copy()
...
# signif_cat_99
column_to_work_with='z_score'

r_df_all_states_pivot_all_columns=r_df_all_states.pivot(index='area_id', columns='class_id').copy()

# df_us gives the amount of people per areal unit, per class.
n_df_us=df_us.set_index(['GISJOIN']).copy()

# Join on indexes
df_n_z_score_all_states=pd.merge(r_df_all_states_pivot_all_columns[column_to_work_with],n_df_us,how='left',left_index=True, right_index=True) 

#multiply number of users per class with z_score per class for each gisjoin, and this for all classes.  
class_id_list=r_df_all_states.class_id.unique()
...
for class_id in class_id_list:
	...
	class_id_x=class_id + '_x' # selected columns from r_df_all_states_pivot we change names later. 
	class_id_y=class_id + '_y' # columns from n_df_us
	class_id_populated=class_id + '_populated_z_score'
	class_id_populated_abs=class_id + '_populated_z_score_abs'

	df_n_z_score_all_states[class_id_populated]=df_n_z_score_all_states[class_id_x]*df_n_z_score_all_states[class_id_y]
	df_n_z_score_all_states[class_id_populated_abs]=df_n_z_score_all_states[class_id_populated].abs()
	...
	rename_dict[class_id_x]=class_id + '_' + column_to_work_with
	rename_dict[class_id_y]=class_id + '_pop'


df_n_z_score_all_states['abs_sum']=df_n_z_score_all_states[columns_with_populated_abs].sum(axis=1)
df_n_z_score_all_states['total_pop']=df_n_z_score_all_states[columns_with_n].sum(axis=1)
df_n_z_score_all_states['weighted_abs_z_score']=df_n_z_score_all_states['abs_sum']/df_n_z_score_all_states['total_pop']
...
for class_id in class_id_list:
    ...
    df_n_z_score_all_states[class_id_pop_percentage]=df_n_z_score_all_states[class_id_y]/df_n_z_score_all_states['total_pop']
    columns_with_pop_percentage.append(class_id_pop_percentage)

df_n_z_score_all_states[columns_with_pop_percentage].round(2)
df_n_z_score_all_states['weighted_abs_z_score'].round(2)

df_n_z_score_all_states.head(5)

# Rename x and y to the correct, n and z_score names
df_n_z_score_all_states=df_n_z_score_all_states.rename(columns=rename_dict)
df_n_z_score_all_states.head(3)


#####################
#4.5 Save dataframe with z_scores, populations, population percentages and weighted abs z-score to pickle
#####################

print 'Starting saving of df_n_z_score_all_states to pickle'
foldername ='/Users/maarten/Desktop/nyu-data/NHGIS/us/'
filename= '%s_censustracts_us_2015/%s_censustracts_us_2015_pickle_n_z_weighted_z_nullmodel_perstate.pkl' %(parameter,parameter)
picklename=foldername+filename

df_n_z_score_all_states.to_pickle(picklename)
print 'Ended saving of df_n_z_score_all_states to pickle'

############################
#5.  Calculate exposure values (repulsion-attraction matrices) per state

# In marble it takes too much time; probably because of the calculation of the variance.
# Remark that Maartens implementation means we have not yet added significancy scores to these calculations. 
############################
...
#####################
'''
def calc_exposure(r_df,n_df, r_areal_unit_name, n_areal_unit_name, alpha, beta):
    print alpha, beta

    #get total number of users in class alpha for all locations 
    total_alpha=n_df[alpha].sum()
...
'''
#####################
#5.2.Prepare dict of n_values per state 
#####################
'''
df_us_tussen3=df_us.copy()
state_list=df_us_tussen3.STATEA.unique()
# We create a dict with as key, the statename and as element the df that we need 
# to feed into marble to make it work. 

print 'Started filling the dict of n_df by state'
n_df_by_state={}

for state in state_list:
    n_df_state=df_us_tussen3.loc[df_us_tussen2['STATEA'] == state].copy()
    n_df_by_state[state]=n_df_state
print 'Ended filling the dict of n_df by state'
'''
#####################
#5.3 Calculate exposure values for each state based on null model per state 
...
#####################
'''
#n_df_by_state gives the amount of people per areal unit, per class. 
#r_df_by_state gives the representation values per class and per areal unit. 

print 'Started the calculation of exposure values per state based on null model per state'
exp_df_by_state={}
for state in r_df_by_state.keys():
	print 'We are currently treating state: ' + str(state)

	# Fill dict with exposure values
	class_id_list= r_df_by_state[state].class_id.unique()
	...
	for class_id in class_id_list:
		for class_id2 in class_id_list:
			exp_dict[class_id][class_id2]=calc_exposure(r_df_by_state[state],n_df_by_state[state],'area_id','GISJOIN',class_id,class_id2)
	print 'Ended calculation of exposure values based on Maartens implementation'

	# Convert dict to dataframe. 
	exp_df=convert_exp_dict_to_df(exp_dict)
	print exp_df
	exp_df_by_state[state]=exp_df

print 'Ended the calculation of exposure values per state based on null model per state'
'''

#####################
#5.4 Unfold exp_df_by_state to one long list and save this to pickle
#####################
'''
#Flush out one large df, collecting the information of all states. 
print 'Started unfolding exp_df_by_states to flush to pickle as one long list'
states_processed=0

for state in exp_df_by_state.keys():
	#Append to masterdf 'exp_df_all_states'
	if states_processed == 0: 
		print 'This is the first state: ' + str(state)
		states_processed=states_processed+1
		exp_df_by_state[state]['STATEA']=state #recreate state field -- we dropped this before to create the marble dicts.
		exp_df_all_states=exp_df_by_state[state]
		print 'resulting shape: ' +str(r_df_all_states.shape)

	else:
		print 'We are adding state: ' + str(state)
		exp_df_by_state[state]['STATEA']=state #recreate state field -- we dropped this before to create the marble dicts.
		exp_df_all_states=exp_df_all_states.append(exp_df_by_state[state], ignore_index=True)
		states_processed=states_processed+1 
		print 'appending shape: ' +str(exp_df_by_state[state].shape)
		print 'resulting shape: ' +str(exp_df_all_states.shape)
print 'Ended unfloding exp_df_by_states to flush to pickle as one long list'


print 'Starting saving of exp_df_all_states to pickle'
foldername ='/Users/maarten/Desktop/nyu-data/NHGIS/us/'
filename= '%s_censustracts_us_2015/%s_censustracts_us_2015_pickle_exp_nullmodel_perstate.pkl' %(parameter,parameter)
picklename=foldername+filename

exp_df_all_states.to_pickle(picklename)
print 'Ended saving of exp_df_all_states to pickle'
'''
